...
const Dashboard = () => {
  ...
  authState?.user?.userId?._id?.toString() === postState?.userId?._id?.toString() ) && (
                      ...
                      <div  onClick={() => {
    if (navigator.share) {
      // Mobile: Use native share dialog (includes Instagram, WhatsApp, etc.)
      navigator
        .share({
      ...
      const shareText = `${post.body} ${window.location.href}`;
      navigator.clipboard.writeText(shareText).then(() => {
        alert("Post copied! You can paste it into Instagram or anywhere.");
      });
    }
  }} className={styles.option}>
